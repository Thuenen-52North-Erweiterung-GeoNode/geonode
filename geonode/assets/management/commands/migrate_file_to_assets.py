    ...
    def handle(self, **options):
        ...
        for asset in LocalAsset.objects.iterator():
            ...
            if resource.subtype == "raster":
                ...
                store_to_update = gs_catalog.get_layer(resource.get_real_instance().alternate).resource.store

                raster_file = [
                    x
                    for x in asset.location
                    if os.path.basename(x).split(".")[-1] in ["tiff", "tif", "geotiff", "geotif"]
                ]
